...
class NetworkCalls {
   ...
   */
  async fetchAllCulversLocations(zipcode: string, wantAllLocations: boolean): Promise<CulversLocation[]> {
    try {
      const response = await axios.get(`https://www.culvers.com/api/locator/getLocations?location=${zipcode}`);
      ...
      return data.geofences.map(location => new CulversLocation(location.metadata));
    } catch (ex) {
   ...
   * @returns {Promise<CulversLocation>}
   */
  public async fetchSingleCulversLocation(zipcode: string): Promise<CulversLocation> {
